...
# rule genotype_discovery_01:
# 	input:
# 		vcf = OUT_FOLDER + "/sv_discovery/{tool}/{sample}/{sample}.{tool}.vcf.gz"
# 	output:
# 		avg_cov_by_readlen = OUT_FOLDER + "/sv_discovery/{tool}/{sample}/{sample}.{tool}.{chrom}.avg_cov_by_readlen", 
# 		vcf = OUT_FOLDER + "/sv_discovery/{tool}/{sample}.{tool}.gt.{chrom}.vcf.gz"
# 	conda:
# 		"../envs/graphtyper.yaml"
# 	params:
# 		bam = OUT_FOLDER + "/input/{sample}.bam",
...
# rule genotype_discovery_02:
# 	input:
# 		processed = expand(OUT_FOLDER + "/sv_discovery/{tool}/{sample}/{sample}.{tool}.gt.{chrom}.vcf.gz", tool=config["TOOLS"], sample=participant_id, chrom=get_chr(OUT_FOLDER + "/merged_cohort/raw_merged.vcf.gz")),
# 		vcf = lambda wildcards: expand(OUT_FOLDER + "/sv_discovery/{tool}/" + wildcards.sample + "/" + wildcards.sample + ".{tool}.gt.{chrom}.vcf.gz", tool=config["TOOLS"], chrom=get_chr(OUT_FOLDER + "/merged_cohort/raw_merged.vcf.gz"))
# 	output:
# 		vcf = OUT_FOLDER + "/sv_discovery/{tool}/{sample}/{sample}.{tool}.gt.vcf",
# 		vcfgz = OUT_FOLDER + "/sv_discovery/{tool}/{sample}/{sample}.{tool}.gt.vcf.gz",
# 		tbi = OUT_FOLDER + "/sv_discovery/{tool}/{sample}/{sample}.{tool}.gt.vcf.gz.tbi"
